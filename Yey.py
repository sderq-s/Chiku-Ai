...
addresses = {}  # عناوين المستخدمين: { user_id: address_str }
...
async def check_answer(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id in challenges and text == challenges[user_id]:
        ...
        await msg.reply_text("تهانينا، لقد فزت بـ20 نجمة!")
...
async def get_address(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id not in addresses:
        addresses[user_id] = str(random.randint(1000000000, 9999999999))
    await update.message.reply_text(f"عنوانك الحالي: {addresses[user_id]}")

# === إرسال النجوم ===
async def send_stars(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text("حسناً، للقيام بذلك؛ أرسل عنوان رفيقك.")

async def receive_address(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    user_id = update.message.from_user.id
    context.user_data["recipient_address"] = update.message.text.strip()
    await update.message.reply_text("الآن، ما هي كمية النجوم المراد إرسالها؟")

async def receive_amount(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    user_id = update.message.from_user.id
    amount = int(update.message.text.strip())

    if balances.get(user_id, 0) < amount:
        await update.message.reply_text("رصيدك غير كافٍ للقيام بهذه العملية.")
    else:
        balances[user_id] -= amount
        await update.message.reply_text(f"تم إرسال {amount} نجمة إلى العنوان {context.user_data['recipient_address']}.")
...
def main() -> None:
    ...
    app.add_handler(MessageHandler(filters.TEXT & filters.ChatType.SUPERGROUP, get_address))
    app.add_handler(MessageHandler(filters.TEXT & filters.ChatType.SUPERGROUP & filters.Regex("^إرسال$"), send_stars))
    app.add_handler(MessageHandler(filters.TEXT & filters.ChatType.SUPERGROUP & filters.Regex("^\d+$"), receive_amount))
    app.add_handler(MessageHandler(filters.TEXT & filters.ChatType.SUPERGROUP & ~filters.Regex("^\d+$"), receive_address))
